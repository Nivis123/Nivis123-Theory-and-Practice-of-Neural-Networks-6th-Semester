...
print("\n" + "=" * 50)
print("ШАГ 7: Сравнение с библиотечной оценкой значимости признаков")
print("=" * 50)

print("\nСравнение Information Gain (InfoGain) и Gain Ratio (все значения в битах):")
print("-" * 100)
print(f"{'Признак':<20} {'IG ручной':<12} {'MI библ (нат)':<14} {'MI библ (бит)':<14} {'IG diff':<12} {'GR ручной':<12} {'GR библ':<12} {'GR diff':<12}")
print("-" * 100)

for name, x in features_for_gain.items():
    ig_manual = info_gain(x, y_target)
    mi_lib_nat = mutual_info_score(x, y_target)
    mi_lib_bit = mi_lib_nat / np.log(2)
    si_lib = entropy_lib(x)
    gr_manual = gain_ratio(x, y_target)
    gr_lib = mi_lib_bit / si_lib if si_lib > 0 else 0

    ig_diff = abs(ig_manual - mi_lib_bit)
    gr_diff = abs(gr_manual - gr_lib)

    print(f"{name:<20} {ig_manual:<12.6f} {mi_lib_nat:<14.6f} {mi_lib_bit:<14.6f} {ig_diff:<12.2e} {gr_manual:<12.6f} {gr_lib:<12.6f} {gr_diff:<12.2e}")
